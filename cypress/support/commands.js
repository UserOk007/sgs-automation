...
Cypress.Commands.add(
  "submitContactForm",
  (inquiryType, subjectOption, typeOfFeedbackOption) => {
    ...
    cy.get(selectors.contact.locationDropDown)
      ...
      .then((options) => {
        ...
        cy.get(selectors.contact.locationDropDown).should(
          "have.value",
          randomOption
        );

        const randomNumbers = faker.datatype
        ...
        if (subjectOption) {
          cy.get(selectors.contact.subject).select(subjectOption);
        }

        if (typeOfFeedbackOption) {
          cy.get(selectors.contact.typeOfFeedback).select(typeOfFeedbackOption);
        }
        ...
        cy.get(selectors.contact.formIsSubmittedSuccessfully).should(
          "be.visible"
        );

        cy.get(selectors.contact.formIsSubmittedSuccessfullyThankYou).should(
          "be.visible"
        );
      });
  }
...
Cypress.Commands.add(
  "selectOptionAndVerifyResults",
  (dropdownId, resultsArea, resultElements, expectedClass) => {
    ...
    cy.get(resultsArea)
      ...
      .then((options) => {
        ...
        cy.wait(3000);
        cy.get(resultElements).each((element) => {
          cy.wrap(element)
            .invoke("text")
            .then((text) => {
...
Cypress.Commands.add(
  "handleDividendScriptCalculator",
  (selectors, testData) => {
...
Cypress.Commands.add('selectRandomLocation', (selector) => {
    cy.get(selector)
      .find('option:not(:first-child)') // Exclude the first option (usually a placeholder)
      .then(options => {
        const randomIndex = Math.floor(Math.random() * options.length); // Generate a random index
        const randomOption = options[randomIndex]; // Select the random option
        cy.wrap(randomOption).invoke('val').then(value => {
          cy.get(selector).select(value); // Select the random option by its value
        });
      });
  });

Cypress.Commands.add(
  "selectFilterVerifyFilterBreadcrumbs",
  ({ filterField, searchValue, dropDown, validOption }) => {
    cy.get(filterField)
      .type(searchValue)
    ...
    cy.get(selectors.search.breadCrumbValue).contains(searchValue);

    cy.get(selectors.search.optionDisplayed).contains(searchValue);
  }
);

Cypress.Commands.add(
  "clearFilterAndVerifyBreadCrumbRemoval",
  (buttonSelector) => {
...
Cypress.Commands.add("verifyFormErrorMessages", (errorMessages) => {
  ...
  errorMessages.forEach((message, index) => {
    cy.get(selectors.contact.fieldInvalidMessage)
      .eq(index)
      .should("be.visible")
      .and("contain", message);
  });
