...
describe("validate Search is working", () => {
  beforeEach("Open home page", () => {
    ...
    cy.get(selectors.search.searchPageTag).should(
      "have.class",
      selectors.search.searchBoxOverlay
    );
  ...
  it(`enter a search word, verify there are 10 results at the page`, () => {
    ...
    cy.get(selectors.search.defaultSortingValue).should(
      "contain",
      testData.searchComponent.defaultSortingValue
    );

    //pagination is represented at the page
    cy.get(selectors.general.padinationArea).should(
      "have.attr",
      selectors.general.attrHidden,
      "false"
    );
  });
  ...
  it(`verify there are no results shown at the page`, () => {
    ...
    cy.get(selectors.search.searchField).type(
      testData.searchComponent.searchNotValidValue
    );
    cy.get(selectors.search.magnifierGlass).last().click();
    cy.get(selectors.search.noResultsText).contains(
      testData.searchComponent.noResults
    );
  });
  ...
  it(`Check whether searching works for filter "Type", "Topic", value is selected and breadcrumb value is correct`, () => {
    ...
    cy.clearFilterAndVerifyBreadCrumbRemoval(
      selectors.search.breadCrumbCloseButton
    );
  });
